...
int main(int argc, char *argv[]) {
    ...
    /*パラメータファイルが指定されていたらそこから読み取って初期化*/
    if (argc >= 6) {
        if (load(argv[4], 50, 784, A1, b1) != 0) {
            return 0;
        }
        if (load(argv[5], 100, 50, A2, b2) != 0) {
            return 0;
        }
        if (load(argv[6], 10, 100, A3, b3) != 0) {
            return 0;
        }
    }
    ...
    for (int i = 0; i < epoch; i++) { 
        ...
        momentum_SGD(train_x, train_y, train_count, n, index, A1, b1, A2,
                     b2,  A3, b3, eta, alpha);
        printf("Epoch %3d/%d\nTrain : ", i + 1, epoch);
        /*訓練データに対する正解率と損失関数の平均*/
        test(A1, b1, A2, b2, A3, b3, train_count, train_x, train_y); 
        printf("Test : ");
        /*テストデータに対する正解率と損失関数の平均*/
        test(A1, b1, A2, b2, A3, b3, test_count, test_x, test_y); 
        printf("\n");
    }
